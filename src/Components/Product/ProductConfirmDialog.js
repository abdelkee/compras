...
function ProductConfirmDialog() {
  const { isOrder, productInfo, user } = useSelector((state) => state.prods);
  ...
  return (
    <div className="z-50 w-4/5 h-60 py-4 bg-white flex flex-col justify-between items-center fixed top-1/2 transform -translate-y-1/2 left-1/2 -translate-x-1/2 shadow-lg rounded-md">
      <button
      ...
      {isOrder ? (
        <span className="font-bold text-xl mt-4 text-indigo-900">
          Confirm Price
        </span>
      ) : (
        <span className="font-bold text-xl mt-14 px-6 text-center text-red-900">
          Want to delete {productInfo.name} ?
        </span>
      )}
